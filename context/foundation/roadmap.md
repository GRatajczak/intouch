...
| S-07 | `account-and-profile-settings` | edit their own profile after first fill and manage their account from `/settings`       | S-01, F-05    | FR-001, FR-002, FR-008 (address), Access Control | in-progress |
...
| F-06       | `product-analytics-posthog`    | PostHog funnel instrumentation for the primary success flow    | yes                   | S-03 done — unblocked. Decide region + capture side before the first event (see Unknowns) |
...
11. **What an analytics event may carry, and where it is stored** — the app's binary privacy guardrail is written about the knowledge base of close ones; an analytics vendor is a second copy of whatever it is handed. Two decisions ride on it: the property allow-list (user id and event name only, or also non-identifying counts like *how many people the user has*), and the account's data region (PostHog Cloud EU / US / self-hosted). Owner: user, during `F-06`'s plan. Block: `F-06` — not hard-blocking, but both are one-way doors once events start flowing.
