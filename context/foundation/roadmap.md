...
| F-07 | `automated-test-harness`       | (foundation) a test runner that gates CI, and an access boundary proven by tests rather than by hand                                        | F-01, S-01       | NFR-privacy, Access Control                         | in-progress (phases 1-4 of 6) |
...
- **Status:** in-progress — Phases 1–4 of 6 complete (`2026-09-07-testing-runner-and-access-boundary`; Phase 2 delivered outside the rollout by `S-05`; Phase 3 as `context/changes/testing-ai-boundary-job-states/`; Phase 4 as `context/changes/testing-input-boundary-and-prompt-composition/`). Phase 6 not started; Phase 5's local gate layer (pre-commit + hooks) is complete but its CI gate is not, and its delivery half is no longer gated since `S-04` shipped.
