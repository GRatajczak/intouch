...
- FR-001: User can create an account and sign in. Priority: must-have
  ...
  > Amended 2026-09-11, `S-17` (`byok-openai-key`): AI cost control now has a second mechanism alongside mandatory login. A user who pastes their own OpenAI API key in `/settings` bills every ranking to that key and faces no cap on manual recomputes; a user without one keeps a free tier capped at one manual "Przelicz teraz" per calendar day (the automatic 24-hour refresh stays unlimited either way). Mandatory login itself is unchanged — this narrows FR-001's cost-control rationale, it does not remove it, since an unauthenticated caller still cannot reach either path.
- FR-002: User can provide a self-profile via a short structured form (name, exact date of birth, a bounded free-text life-context field, and three optional rhythm selectors — weekly time budget for staying in touch, preferred contact channels, and the parts of the week they have space for it) so the AI has clean context about who is asking and what a realistic suggestion looks like for them. Priority: must-have
  ...
  > Amended 2026-08-30 during `S-01` implementation: age moved from a 3-bucket range (20s/30s/40s+) to an exact birth date, for precision the app can compute from directly. Life context moved from a fixed 4-option enum to a bounded free-text field (max 300 chars, UI shows an example via tooltip) — a partial return to free text for this one field, because the enum's real-world coverage was too narrow (a user's actual situation rarely matches one of 4 fixed buckets) to be worth the structure. A short bounded paragraph stays fast to fill and gives the AI richer signal than a forced-fit bucket.
  > Amended 2026-08-31, planning `S-09` (`self-profile-rhythm-fields`): three optional fields added — weekly time budget (under 1h / 1–3h / over 3h), preferred channels (phone / message / in person / video, multi-select), and availability windows (weekday morning / weekday evening / weekend, multi-select). Taken from the design bundle's onboarding card (`.ai/intouch-design-preparation/project/InTouch.dc.html:87-135`), which introduced them with no PRD backing. They are the sole source of the design's `Twój rytm` explanation chip: without them FR-007's hierarchy can say *who* and *how urgent* but not *how* or *when*, and has no cap on how many suggestions one week can absorb. Deliberately **optional** — a profile filled before this amendment stays valid and its owner is never forced back through the form, so FR-007 must degrade gracefully when they are empty. The design's 3-step onboarding wizard and its `Push` channel were **not** adopted (FR-008 is email-only).
...
2. **Structured-form fields** — the **self-profile half is closed**: FR-002's field set was pinned during `S-01` (name, birth date, life context) and completed during `S-09` (the three optional rhythm selectors). The **per-person half (FR-003) is still open** — the design bundle shows a separate `Kim jest dla Ciebie?` free-text, a chip list for `Co go cieszy, co jest u niego ważne?`, a `Kategoria` selector (an FR-006 concern, parked) and a `Kiedy ostatnio rozmawialiście?` bucket that has no counterpart in the schema, while the shipped form collapses the first two into one `Opis` textarea. Owner: user, during design.
